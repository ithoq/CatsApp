'use strict';

angular.module('app').controller('AppCtrl', ['$scope', '$rootScope', '$state', 'Restangular', '$translate', 'localStorageService', '$http', function($scope, $rootScope, $state, Restangular, $translate, localStorageService, $http) {

    // App globals
    $scope.app = {
        name: 'Ignition',
        description: 'Ignition Dashboard',
        layout: {
            menuPin: false,
            menuBehind: false,
            theme: 'pages/css/pages.css'
        },
        author: ''
    }

    $scope.rest = {
        login: Restangular.all('login/'),
        teamCandidates : Restangular.all('candidates/')
    }

    var token = localStorageService.get('token');
    if (token != null) 
        $http.defaults.headers.common['Authorization'] = 'Token ' + token;


    $translate.use('fr');

    // Checks if the given state is the current state
    $scope.is = function(name) {
        return $state.is(name);
    }

    // Checks if the given state/child states are present
    $scope.includes = function(name) {
        return $state.includes(name);
    }

    // Broadcasts a message to pgSearch directive to toggle search overlay
    $scope.showSearchOverlay = function() {
        $scope.$broadcast('toggleSearchOverlay', {
            show: true
        })
    }

}]);

/*
    Use this directive together with ng-include to include a 
    template file by replacing the placeholder element
*/
angular.module('app').directive('includeReplace', function() {
    return {
        require: 'ngInclude',
        restrict: 'A',
        link: function(scope, el, attrs) {
            el.replaceWith(el.children());
        }
    };
});